...
public class CallbackReference extends WeakReference<Callback> {
    ...
    static {
        if (Platform.isWindows()) {
            try {
                DLL_CALLBACK_CLASS = Class.forName("jp.hisano.wasm.easybind.win32.DLLCallback");
    ...
    static Class<?> findCallbackClass(Class<?> type) {
        if (!Callback.class.isAssignableFrom(type)) {
            throw new IllegalArgumentException(type.getName() + " is not derived from jp.hisano.jna4wasm.jna.Callback");
        }
        if (type.isInterface()) {
            return type;
        }
